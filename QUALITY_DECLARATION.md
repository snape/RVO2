...
- [`.github/workflows/ci.yml`](.github/workflows/ci.yml): builds and tests
  with CMake, Bazel, and Meson on AlmaLinux, Arch Linux, Fedora, openSUSE,
  and Ubuntu (amd64 and arm64), and macOS (arm64); builds and tests with
  CMake and Meson on Alpine Linux (Bazel is not tested on Alpine due to musl
  libc incompatibility). CMake builds enable `BUILD_TESTING`,
  `ENABLE_HARDENING`, `ENABLE_OPENMP`, and `WARNINGS_AS_ERRORS`. Meson
  builds enable `hardening`, `openmp`, and `werror`.
...
documentation is generated by CMake (`-DBUILD_DOCUMENTATION=ON`) or
Meson (`-Ddocs=true`).
...
Copyright is held by the University of North Carolina at Chapel Hill, 2008. All
...
`RVO2 Library` is not a ROS package and has no direct or indirect ROS runtime
dependencies.
...
|----------------|--------------|---------------------|
| AlmaLinux 10   | amd64        | CMake, Bazel, Meson |
