...
- [`.github/workflows/muon.yml`](.github/workflows/muon.yml): builds and
  tests with [muon](https://muon.build) (a C reimplementation of the Meson
  build system) on AlmaLinux, Arch Linux, Fedora, openSUSE, Ubuntu (amd64
  and arm64), Alpine Linux, and macOS (arm64); also enforces meson.build file
  formatting via `muon fmt`.
...
- **muon fmt**: Meson file formatting enforced by the `muon-fmt` job in
  [`.github/workflows/muon.yml`](.github/workflows/muon.yml)
